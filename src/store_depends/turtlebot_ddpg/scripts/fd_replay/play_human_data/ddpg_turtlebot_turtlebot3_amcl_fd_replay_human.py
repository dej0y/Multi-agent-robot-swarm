...
# code used during human assisted training 
vertices = [(7, 8), (7, -6.2), (-6.5, -6.2), (-6.5, 1.25), (-12.5, 1.5), (13.5, 7.5)]
...
class GameState:
    ...
    def __init__(self):
        ...
        self.pub = rospy.Publisher('/bot1/cmd_vel', Twist, queue_size=1)
        self.position = Point()
        self.move_cmd = Twist()
        ...
        self.odom_frame = '/bot1/odom'
        try:
            self.tf_listener.waitForTransform(self.odom_frame, '/bot1/base_footprint', rospy.Time(), rospy.Duration(1.0))
            self.base_frame = '/bot1/base_footprint'
        except (tf.Exception, tf.ConnectivityException, tf.LookupException):
            try:
                self.tf_listener.waitForTransform(self.odom_frame, '/bot1/base_link', rospy.Time(), rospy.Duration(1.0))
                self.base_frame = '/bot1/base_link'
            except (tf.Exception, tf.ConnectivityException, tf.LookupException):
    ...
    def reset(self):
        ...
        random_points = self.generate_random_points_in_polygon(polygon, num_points=4)
        self.target_x = random_points[0][0]
        self.target_y = random_points[0][1]
        
        
        self.target_a = random_points[3][0]
        self.target_b = random_points[3][1]
        ...
        state_msg.model_name = 'bot1'
        state_msg.pose.position.x = self.target_a
        state_msg.pose.position.y = self.target_b #random_turtlebot_y
    ...
    def generate_random_points_in_polygon(self, polygon, num_points=1):
        points = []
    ...
    def game_step(self, time_step=0.1, linear_x=0.8, angular_z=0.3):
        ...
        record_time_step = 0
        self.move_cmd.linear.x = linear_x*0.5
        ...
        state = np.append(state, linear_x*0.5)
